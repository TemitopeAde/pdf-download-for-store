...
const Panel: FC = () => {
  ...
  return <WixDesignSystemProvider><SidePanel width="320" height="100vh"><SidePanel.Content noPadding stretchVertically><Box direction="vertical" gap="SP4" padding="SP4"><Text appearance="H2">Product downloads</Text><Text secondary>Each assigned file appears as a button. Set its label when assigning or uploading the file; if no label is set, the button says Download.</Text><FormField label="Button layout"><Dropdown size="small" options={[{ id: 'STACK', value: 'One per row' }, { id: 'ROW', value: 'Side by side' }]} selectedId={settings.layout} valueParser={(option) => option.value} onSelect={(option) => set('layout', option.id === 'ROW' ? 'ROW' : 'STACK')} /></FormField><FormField label="Label font"><Button priority="secondary" fullWidth onClick={chooseFont}><Text size="small" ellipsis>{settings.labelFont.font === 'system-ui' ? 'Choose font' : settings.labelFont.font}</Text></Button></FormField><FormField label="Label color"><Box width="30px" height="30px"><FillPreview fill={settings.labelColor} onClick={chooseColor} /></Box></FormField></Box></SidePanel.Content></SidePanel></WixDesignSystemProvider>;
};
